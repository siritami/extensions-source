...
internal class GenreFilter : Filter.Group<Filter.CheckBox>(
    "Tag",
    GENRE_OPTIONS.map { Filter.CheckBox(it.first, false) },
) {
    fun selectedValues(): List<String> {
        return state
...
internal class StatusFilter : UriPartFilterNullable(
    "Trạng thái",
    STATUS_OPTIONS,
)

internal class SortByFilter : UriPartFilter(
    "Sắp xếp",
    SORT_BY_OPTIONS,
)

internal class SortOrderFilter : UriPartFilter(
    "Thứ tự",
    SORT_ORDER_OPTIONS,
)
