...
internal object ImgxCrypto {
    ...
    fun decodeImgxV3(encrypted: ByteArray, grant: ImgxGrant, storageKey: String): ByteArray {
        require(encrypted.size > 41 && encrypted[4].toInt() == 3) { "IMGX v3 payload invalid" }
        val width = readUint32(encrypted, 5)
        val height = readUint32(encrypted, 9)
    ...
    fun extractImx4FromWebp(bytes: ByteArray): ByteArray? {
    ...
    fun decodeProtectedPage(encrypted: ByteArray, grant: ImgxGrant, storageKey: String): ByteArray {
        val version = encrypted[4].toInt()
        if (version == 4) {
            val key = unwrapGrantKey(grant, storageKey, "wrappedV4Key")
            try {
                return decodeImgxV4P01(encrypted, key, grant.imageId.orEmpty(), storageKey)
            } finally {
                key.fill(0)
            }
        }
        if (version != 3) throw IllegalStateException("IMGX version unsupported: $version")
