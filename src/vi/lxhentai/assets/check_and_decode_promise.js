(function(){
    try {
        ...
        if (!window._lxHooked) {
            ...
            window._lxTokenResolved = false;
            window._lxTokenPromise = new Promise(function(resolve) {
                window._lxTokenResolve = resolve;
            });
            
            var origFetch = window.fetch.bind(window);
            window.fetch = function(input, init) {
                var url = (typeof input === 'string') ? input : (input && input.url) || '';
                _dbg.push('FETCH:' + url.substring(0, 80));
                if (url.indexOf('get_token') >= 0) {
                    _dbg.push('HOOK:get_token intercepted');
                    return origFetch(input, init).then(function(resp) {
                        _dbg.push('HOOK:resp status=' + resp.status);
                        if (resp.status === 200 && !window._lxTokenResolved) {
                            var clone = resp.clone();
                            clone.text().then(function(txt) {
                                _dbg.push('HOOK:body=' + txt.substring(0, 200));
                                try {
                                    var data = JSON.parse(txt);
                                    window._lxToken = data.action_token || null;
                                    window._lxTokenResolved = true;
                                    window._lxTokenResolve(window._lxToken);
                                    _dbg.push('HOOK:tokenSet=' + (window._lxToken ? 'yes' : 'no'));
                                } catch(pe) { 
                                    _dbg.push('HOOK:parseErr=' + pe);
                                    window._lxToken = null;
                                    window._lxTokenResolved = true;
                                    window._lxTokenResolve(null);
                                }
                            }).catch(function(ce) { 
                                _dbg.push('HOOK:clone err=' + ce);
                                window._lxToken = null;
                                window._lxTokenResolved = true;
                                window._lxTokenResolve(null);
                            });
            ...
            _dbg.push('hookInstalled=ok');
        } else {
            _dbg.push('hookAlready=ok');
        }
        ...
        // 5. Wait for token to be available
        if (!window._lxTokenResolved) {
            window._lxTokenPromise.then(function(token) {
                window._lxToken = token;
                window._lxTokenResolved = true;
            });
        }
